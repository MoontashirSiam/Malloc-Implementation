...
static metadata_t *find_right(metadata_t *freed_block) {
    if (!freed_block) return NULL;
    metadata_t *curr = address_list;
    while (curr) {
        if ((uintptr_t) curr == (uintptr_t) ((uint8_t*) (freed_block + 1) + freed_block->size)) {
            return curr;
        }
        curr = curr->next;
    }
...
static metadata_t *split_block(metadata_t *block, size_t size) {
    int newBlockSize = TOTAL_METADATA_SIZE + size;
    int newBlockStart = TOTAL_METADATA_SIZE + block->size - newBlockSize;
    block->size = block->size - newBlockSize;
    metadata_t *newBlock = (metadata_t *)((uint8_t *)block + newBlockStart);\
    newBlock->size = size;
    newBlock->next = block->next;
    block->next = newBlock;
    // if (block == address_list) {
    //     address_list->size = address_list->size - newBlockSize;
    // }
    return newBlock;
}
...
static void add_to_addr_list(metadata_t *block) {
    ...
    if (!curr) {
        address_list = block;
    } else if ((uintptr_t) (curr) > (uintptr_t) block) {
          address_list = block;
          block->next = curr;
    } else{
...
static metadata_t *find_best_fit(size_t size) {
    ...
    while (curr) {
        if (curr->size == size) {
            best_fit = curr;
            // best_fit->next = curr->next;
            // best_fit->size = curr->size;
            // remove_from_addr_list(curr);
            return best_fit; 
        }
        if (curr->size > size && best_fit == NULL) {
            best_fit = curr;
        } else if (curr->size > size && best_fit->size > curr->size) {
            best_fit = curr;
        }
        curr = curr->next;
    }
...
void *my_malloc(size_t size) {
    ...
    if (!best_fit) {
        // 3.a.
        void *block = my_sbrk(SBRK_SIZE);
        // 3.b.
        if (block == (void *)-1) {
            my_malloc_errno = OUT_OF_MEMORY;
            return NULL;
        }
        metadata_t *new_block = (metadata_t *)block;
        new_block->next = NULL;
        new_block->size = SBRK_SIZE - TOTAL_METADATA_SIZE;
        // 3.c.
        add_to_addr_list(new_block);
        metadata_t *right = find_right(new_block);
        if (right) { 
            merge(new_block, right);
            // remove_from_addr_list(right);
        }
        metadata_t *left = find_left(new_block);
        if (left) {
            merge(left, new_block);
            // remove_from_addr_list(new_block);
        }
        // 3.d
    ...
    // 5 - 5.a.
    metadata_t *newBlock = split_block(best_fit, size);
